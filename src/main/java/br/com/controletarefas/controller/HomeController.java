...
public class HomeController {
    ...
    @GetMapping("/")
    String inicio(Authentication auth) {
        return auth.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals("ROLE_SUPERVISOR"))
                ? "redirect:/dashboard"
                : "redirect:/minhas-tarefas";
    }
    ...
    String minhasTarefas(Authentication auth, Model model) {
        model.addAttribute("atribuicoes", atribuicaoService.buscarDoUsuarioAutenticado(auth.getName()));
