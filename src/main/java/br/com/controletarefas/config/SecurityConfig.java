        ...
        SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
                return http
                                .authorizeHttpRequests(auth -> auth
                                                ...
                                                .requestMatchers("/dashboard", "/usuarios/**", "/tarefas/**",
                                                                "/atribuicoes/**",
                                                                "/tipos-tarefa/**", "/clientes/**")
                                                .hasRole("SUPERVISOR")
                                                .requestMatchers("/minhas-tarefas").hasAnyRole("SUPERVISOR", "OPERADOR")
                                                .anyRequest().authenticated())
                                .formLogin(form -> form.loginPage("/login")
                                                .successHandler((request, response, authentication) -> {
                                                        boolean supervisor = authentication.getAuthorities().stream()
                                                                        .anyMatch(a -> a.getAuthority()
                                                                                        .equals("ROLE_SUPERVISOR"));
                                                        response.sendRedirect(
                                                                        supervisor ? "/dashboard" : "/minhas-tarefas");
